...
public:
  ...
  : Node("aruco_detector_node")
  {
    ...
    this->declare_parameter<double>("marker_size_m", 0.2);
    ...
    confidence_reproj_scale_px_ = static_cast<float>(this->get_parameter("confidence_reproj_scale_px").as_double());
    confidence_min_marker_span_px_ = static_cast<float>(this->get_parameter("confidence_min_marker_span_px").as_double());
    confidence_full_marker_span_px_ = static_cast<float>(this->get_parameter("confidence_full_marker_span_px").as_double());
    ...
    controller_state_sub_ = this->create_subscription<uav_visual_landing::msg::LandingControllerState>(
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "aruco_detector_node: image=%s camera_info=%s target_observation=%s debug_image=%s marker_id=%d marker_size=%.3f",
      image_topic_.c_str(),
...
private:
  ...
  {
    const float half = marker_size_m_ * 0.5f;
    return {
      {-half, half, 0.0f},
      {half, half, 0.0f},
      {half, -half, 0.0f},
  ...
  {
    candidates.clear();
    if (corners.size() != 4 || marker_size_m_ <= 1.0e-6f || !camera_info_received_) {
      return false;
    }
    ...
    try {
      ...
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "solvePnPGeneric failed: %s", e.what());
    }
    ...
    if (rvecs.empty() || tvecs.empty()) {
      ...
      try {
        ...
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "solvePnP failed: %s", e.what());
        return false;
      }
    ...
    for (size_t index = 0; index < std::min(rvecs.size(), tvecs.size()); ++index) {
      ...
      candidate.yaw_rad = computePoseYawRad(candidate.rvec);
      candidate.reproj_err_px = computeReprojectionErrorPx(object_points, corners, candidate.rvec, matToVec3d(tvecs[index]));
      candidates.push_back(candidate);
    }
    ...
    const uav_visual_landing::msg::TargetObservation & observation)
  {
    ...
    cv::Mat debug_bgr;
    cv::cvtColor(frame, debug_bgr, cv::COLOR_GRAY2BGR);
    if (!corners.empty()) {
    ...
    auto draw_line = [&](const std::string & text)
    {
      cv::putText(
        ...
        font_scale, cv::Scalar(20, 20, 20), thickness + 2, cv::LINE_AA);
      cv::putText(
        debug_bgr, text, cv::Point(12, y), cv::FONT_HERSHEY_SIMPLEX,
    ...
    draw_line("VISUAL LANDING DEBUG");
    draw_line(std::string("PHASE: ") + last_phase_);
    draw_line(observation.detected ? "TAG: DETECTED" : "TAG: MISSING");
    draw_line(
      cv::format(
        "ERR px=(%.1f, %.1f) n=(%.4f, %.4f)",
        observation.pixel_err_u, observation.pixel_err_v,
        observation.err_u_norm, observation.err_v_norm));
    draw_line(
      cv::format(
        "YAW=%.3f conf=%.2f rep=%.2f span=%.1f",
        observation.yaw_err_rad, observation.confidence,
        observation.reproj_err_px, observation.marker_span_px));

    const auto output_msg = cv_bridge::CvImage(msg->header, sensor_msgs::image_encodings::BGR8, debug_bgr).toImageMsg();
  ...
  void onImage(const sensor_msgs::msg::Image::SharedPtr msg)
  {
    ...
    cv::Mat frame;
    try {
      frame = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::MONO8)->image;
    ...
    cv::aruco::detectMarkers(frame, tag_dict_, corners, ids, tag_params_);
    ...
    if (target_index < 0) {
      ...
      publishDebugImage(msg, frame, corners, ids, target_index, observation);
      return;
    }
  ...
  rclcpp::Subscription<uav_visual_landing::msg::LandingControllerState>::SharedPtr controller_state_sub_;
  rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr image_sub_;
  rclcpp::Publisher<uav_visual_landing::msg::TargetObservation>::SharedPtr observation_pub_;
  rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr debug_image_pub_;
  std::string last_phase_{"READY"};
};
