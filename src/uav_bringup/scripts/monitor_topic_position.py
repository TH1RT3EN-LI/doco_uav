...
class PositionMonitorGui:
    ...
    def _build_window(self):
        self.root.title("OpenVINS / UAV 融合状态 / PX4 位置实时监视")
        ...
        tk.Label(
            header,
            text="OpenVINS / UAV 融合状态 / PX4 位置实时监视",
            font=("Sans", 22, "bold"),
            fg="#f8fafc",
            bg="#0f172a",
        ).pack(anchor="w")
        tk.Label(
            header,
            text=(
                "默认对比 OpenVINS 原始输出、当前 /uav/state/odometry_px4，以及 PX4 原始输出；"
                "单位统一为米（m）。"
            ),
            font=("Sans", 11),
            fg="#cbd5e1",
    ...
    def _refresh_widgets(self):
        ...
        for state in self.source_states:
            ...
            freshness_s = (
                0.0
                if last_receive_wall_s is None
                else max(now - last_receive_wall_s, 0.0)
            )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GUI 方式实时监视 OpenVINS 原始输出、当前 UAV 融合状态与 PX4 原始输出的位置。"
    )
    parser.add_argument(
        "--sources",
